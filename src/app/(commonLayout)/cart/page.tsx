...
const CartPage = () => {
  ...
  if (cart.length === 0) {
    return (
      <div className="container mx-auto p-8 text-center text-gray-500 text-lg">Your cart is empty.</div>
    );
  }
  ...
  return (
    <div className="container mx-auto p-8 ">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4 gap-6 mb-10">
        {cart.map((product) => (
          <CartPageCard key={product.id} product={product} onRemove={removeFromCart} />
        ))}
      </div>
    </div>
  );
